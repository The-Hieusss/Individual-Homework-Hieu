...
//		    statement.execute("DROP TABLE IF EXISTS Questions");
	    	// Create Questions table
	    	  // Updated Questions table: store the asker's name as VARCHAR instead of a numeric id
	        ...
	        // Updated Answers table: remove sId column; use answerFrom for the answerer's name
	        ...
	        // Updated Reviews table: remove sId column; use reviewFrom for the reviewer's name
	...
	public boolean isDatabaseEmpty() throws SQLException {
	 ...
	 // Update question remains similar, now using question id
	...
	public void deleteReview(int rId) throws SQLException {
	 ...
	 public void createReview(int aId, String reviewFrom, String reviewText) throws SQLException {
	        String query = "INSERT INTO Reviews (aId, reviewFrom, reviewText) VALUES (?, ?, ?)";
	        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	            pstmt.setInt(1, aId);
	            pstmt.setString(2, reviewFrom);
    ...
    public void createAnswer(int qId, String answerFrom, String answerText) throws SQLException {
        String query = "INSERT INTO Answers (qId, answerFrom, answerText) VALUES (?, ?, ?)";
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            pstmt.setInt(1, qId);
            pstmt.setString(2, answerFrom);
	...
	//gets all questions
	  // Simple query: Get all questions with the asker's name (no filtering)
    public ResultSet getAllQuestions() throws SQLException {
        String query = "SELECT id, asker, qTitle, qDesc FROM Questions";
        return statement.executeQuery(query);
    }
    
    // Add question now accepts the asker's name (String)
    public void addQuestion(String asker, String qTitle, String qDesc) throws SQLException {
        String query = "INSERT INTO Questions (asker, qTitle, qDesc) VALUES (?, ?, ?)";
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            pstmt.setString(1, asker);
	...
	// Get all answers for the staffs
	...
	// Get all direct messages from people
